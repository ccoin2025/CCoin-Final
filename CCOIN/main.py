...
app.add_middleware(SessionMiddleware, secret_key=SECRET_KEY, https_only=True if ENV == "production" else False)
...
async def verify_telegram_init_data(request: Request):
    ...
    # Verify init_data with Telegram's method
    bot_token_hash = secrets.token_hex(32)  # Simplified; in production, use HMAC-SHA256 with BOT_TOKEN
...
async def airdrop_tokens(request: Request, db: Session = Depends(get_db)):
    ...
    async with AsyncClient(SOLANA_RPC) as client:
        admin_keypair = Keypair()  # In production, load from secure storage
